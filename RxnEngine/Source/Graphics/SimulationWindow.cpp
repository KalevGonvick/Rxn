...
namespace Rxn::Graphics
{
    ...
    void SimulationWindow::InitializeRender()
    {
...

        {
            ...
            GetScene().InitSceneRenderTargets(GetDisplay().GetFrameIndex(), GetDisplay().GetSwapChain());
            GetCommandListManager().CreateCommandList(GOHKeys::CmdList::PRIMARY, currentCommandAllocator);
            
            ThrowIfFailed(GetCommandListManager().GetCommandList(GOHKeys::CmdList::PRIMARY)->Close());
            ID3D12CommandList *ppCommandLists[] = { GetCommandListManager().GetCommandList(GOHKeys::CmdList::PRIMARY).Get() };
            primaryCommandQueue->ExecuteCommandLists(_countof(ppCommandLists), ppCommandLists);

           
        }

        GetCommandListManager().CreateCommandList(GOHKeys::CmdList::INIT, currentCommandAllocator);
        ...
        {
            GetScene().GetDynamicConstantBuffer().Init(RenderContext::GetGraphicsDevice().Get());

            // Close the command list and execute it to begin the initial GPU setup.
            ThrowIfFailed(initCmdList->Close());
            ID3D12CommandList *ppCommandLists[] = { initCmdList };
            primaryCommandQueue->ExecuteCommandLists(_countof(ppCommandLists), ppCommandLists);
        ...
        GetFence().MoveFenceMarker(primaryCommandQueue, GetDisplay().GetFrameIndex(), nextFrameIndex);
        ...
        GetFence().SignalFence(primaryCommandQueue, GetDisplay().GetFrameIndex());
        GetFence().WaitInfinite(GetDisplay().GetFrameIndex());
        GetFence().IncrementFenceValue(GetDisplay().GetFrameIndex());
