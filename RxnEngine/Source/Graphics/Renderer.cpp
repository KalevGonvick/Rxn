...
namespace Rxn::Graphics
{
    ...
    HRESULT Renderer::CreateVertexBufferResource()
    {
        ...
        m_Scene.AddShapeFromRaw(cubeVertices, cubeIndices, m_CommandAllocators[m_Display.GetFrameIndex()].Get(), m_CommandQueueManager.GetCommandQueue(GOHKeys::CmdQueue::PRIMARY).Get(), m_CommandListManager.GetCommandList(GOHKeys::CmdList::INIT).Get());

        HRESULT result;

        /*result = m_Shape.UploadGpuResources(RenderContext::GetGraphicsDevice().Get(), m_CommandQueueManager.GetCommandQueue(GOHKeys::CmdQueue::PRIMARY).Get(), m_CommandAllocators[m_Display.GetFrameIndex()].Get(), m_CommandListManager.GetCommandList(GOHKeys::CmdList::INIT).Get());
        if (FAILED(result))
        {
        ...
        m_Scene.AddQuadFromRaw(quadVertices, m_CommandAllocators[m_Display.GetFrameIndex()].Get(), m_CommandQueueManager.GetCommandQueue(GOHKeys::CmdQueue::PRIMARY).Get(), m_CommandListManager.GetCommandList(GOHKeys::CmdList::INIT).Get());
        /*m_Quad.ReadDataFromRaw(quadVertices);

        result = m_Quad.UploadGpuResources(RenderContext::GetGraphicsDevice().Get(), m_CommandQueueManager.GetCommandQueue(GOHKeys::CmdQueue::PRIMARY).Get(), m_CommandAllocators[m_Display.GetFrameIndex()].Get(), m_CommandListManager.GetCommandList(GOHKeys::CmdList::INIT).Get());
        if (FAILED(result))
        {
